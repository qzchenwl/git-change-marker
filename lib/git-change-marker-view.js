...
export default class GitChangeMarkerView {
  ...
  constructor(editor) {
    ...
    this.subscriptions.add(gutterView, 'click', '.git-line-added, .git-line-removed, .git-line-modified', e => {
      ...
      // dynamic fit width & height
      setTimeout(() => {

        const diffEditor  = toolbar[0].querySelector('atom-text-editor');
        const diffContent = diffEditor.querySelector('.scroll-view > div');
        const diffGutter  = diffEditor.querySelector('.gutter-container');
        const contentRect = diffContent.getBoundingClientRect();
        const gutterRect  = diffGutter.getBoundingClientRect();

        const width = Math.min(contentRect.width + gutterRect.width, editor.element.getWidth());
        const height = Math.min(window.innerHeight - contentRect.top, contentRect.height);
        diffEditor.style.width = width + 'px';
        diffEditor.style.height = height + 'px';
