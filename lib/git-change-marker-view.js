        ...
        onRevert: () => {
          ...
          git.getRepoForFile(path).then(repo =>
            Promise.all([repo.getLineDiffs(path, this.editor.getText()), git.getHeadContent(repo, path)]).then(([diffs, head]) => {
              const {newStart, newLines, oldStart, oldLines} = diffs.filter(({newStart, newLines}) => newStart <= row + 1 && newStart + newLines > row + 1);
              const lines = head.split('\n');
              const oldContent = lines.slice(oldStart - 1, oldLines).join('\n');
              this.editor.setTextInBufferRange([[newStart, 0], [newStart + newLines - 1, Number.MAX_VALUE]], oldContent);
            }));
        },
