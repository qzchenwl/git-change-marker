  ...
  constructor(editor) {
    ...
    this.subscriptions.add(gutterView, 'click', '.git-line-added, .git-line-removed, .git-line-modified', e => {
      ...
      const controlView = new ControlView({
        onPreviousDiff: () => this.moveToPreviousDiff(),
        onNextDiff: () => this.moveToNextDiff(),
      ...
      controlView.on('click', destroyDecoration);
      controlView.on('mousedown', e => e.stopPropagation());

      editor.decorateMarker(this.marker, {type: 'overlay', item: controlView});
    });
  }
